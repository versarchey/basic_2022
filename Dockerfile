...
RUN cd /tmp/swig && wget https://ftp.exim.org/pub/pcre/pcre2-10.37.tar.gz && bash Tools/pcre-build.sh
#RUN cd /tmp/swig && wget https://ftp.exim.org/pub/pcre/pcre2-10.37.tar.gz && bash Tools/pcre-build.sh
...
WORKDIR $BASIC_DIR
RUN /usr/bin/python2.7 setup.py $BASIC_DIR/extsds-bin $BASIC_DIR/pytools/ --python=$BASIC_DIR/_py
